...
void InsertNumber::runCommand() {
    if (!TableHandler::tableVector.empty()) {
        ...
        vectorIndex = Utils::provideAnInt(0, TableHandler::tableVector.size() - 1);
        cout << "Now provide Table's index you want to assign to" << endl;
        index = Utils::provideAnInt(0, TableHandler::tableVector.at(vectorIndex)->getSize() - 1);
...
bool InsertNumber::setValueInTable(int vectorIndex, int index, int newValue) {
    if (vectorIndex >= 0 && vectorIndex < TableHandler::tableVector.size()) {
        return TableHandler::tableVector[vectorIndex]->setVaule(index, newValue);
    } else return false;
}
